...
#        ufs = np.zeros(s['zb'].shape)
#        ufs[:,1:-1] = 0.5*s['uus'][:,:-2,0] + 0.5*s['uus'][:,2:,0]
#        ufn = np.zeros(s['zb'].shape)
#        ufn[1:-1,:] = 0.5*s['uun'][:-2,:,0] + 0.5*s['uun'][2:,:,0]
        # boundary values
#        ufs[:,0]  = s['uus'][:,0,0]
#        ufs[:,-1] = s['uus'][:,-1,0]
...
#                print("iteration nr = %d" % n)
                ...
                if qflag :
                    ...
                    qsxfs_i = np.zeros(ufs.shape)
                    qsxfn_i = np.zeros(ufn.shape)
                    qsxfs_i[:,1:-1] = ixfs[:,1:-1] * ( alpha * qs[:,:-1,i] \
                                                      + (1. - alpha ) * l['qs'][:,:-1,i] ) \
                        + (1. - ixfs[:,1:-1]) * ( alpha * qs[:,1:,i] \
                                                 + (1. - alpha ) * l['qs'][:,1:,i] )
                    qsxfn_i[1:-1,:] = ixfn[1:-1,:] * (alpha * qs[:-1,:,i] \
                                                      + (1. - alpha ) * l['qs'][:-1,:,i] ) \
                        + (1. - ixfn[1:-1,:]) * ( alpha * qs[1:,:,i] \
                                                 + (1. - alpha ) * l['qs'][1:,:,i] )
                    if p['boundary_lateral'] == 'circular':
                        qsxfn_i[0,:] = ixfn[0,:] * (alpha * qs[-1,:,i] \
                                                    + (1. - alpha ) * l['qs'][-1,:,i] ) \
                            + (1. - ixfn[0,:]) * ( alpha * qs[0,:,i] \
                                                   + (1. - alpha ) * l['qs'][0,:,i] )
                        qsxfn_i[-1,:] = qsxfn_i[0,:]                   
                    # sediment flux in y-direction
                    qnxfs_i = np.zeros(ufs.shape)
                    qnxfn_i = np.zeros(ufn.shape)
                    qnxfs_i[:,1:-1] = ixfs[:,1:-1] * ( alpha * qn[:,:-1,i] \
                                                      + (1. - alpha ) * l['qn'][:,:-1,i] ) \
                        + (1. - ixfs[:,1:-1]) * ( alpha * qn[:,1:,i] \
                                                 + (1. - alpha ) * l['qn'][:,1:,i] )
                    qnxfn_i[1:-1,:] = ixfn[1:-1,:] * (alpha * qn[:-1,:,i] \
                                                      + (1. - alpha ) * l['qn'][:-1,:,i] ) \
                        + (1. - ixfn[1:-1,:]) * ( alpha * qn[1:,:,i] \
                                                 + (1. - alpha ) * l['qn'][1:,:,i] )
                    if p['boundary_lateral'] == 'circular':
                        qnxfn_i[0,:] = ixfn[0,:] * (alpha * qn[-1,:,i] \
                                                    + (1. - alpha ) * l['qn'][-1,:,i] ) \
...
#                    yqn_i         += s['uun'][:,:] * E_i - ugn[:,:] * D_i       #source term
                ...
                if p['boundary_onshore'] == 'noflux':
                    ...
                    yCt_i[:,-1]   += s['dnz'][:,-1]  * ufs[:,-1] * s['Cu0'][:,-1,i] #upper x-face
                    if qflag :
                    ...
                    #remove the flux at the inner face of the cell
                    yCt_i[:,-1]  -= s['dnz'][:,-2] * ufs[:,-2] * Ctxfs_i[:,-2] #lower x-face
                    if qflag :
                ...
                if p['boundary_lateral'] == 'noflux':
                    ...
                    yCt_i[0,:]   += s['dsz'][0,:] * ufn[0,:]  * s['Cu0'][0,:,i] * p['sedimentinput'] #lower y-face
                    yCt_i[-1,:]  -= s['dsz'][-1,:] * ufn[-1,:] * s['Cu0'][-1,:,i] * p['sedimentinput'] #upper y-face
                    
#                    yCt_i[0,:]   += s['dsz'][0,:] * s['Cu0'][0,:,i] * p['sedimentinput'] #lower y-face
#                    yCt_i[-1,:]  -= s['dsz'][-1,:] * s['Cu0'][-1,:,i] * p['sedimentinput'] #upper y-face
...
#                    jp = ufn[-1,:] >  0
#                    
#                    yCt_i[0,im]  += s['dsz'][0,im] * ufn[0,im] * s['Cu0'][0,im,i] * p['sedimentinput']  #lower y-face
#                    yCt_i[0,ip]  += s['dsz'][1,ip] * ufn[1,ip] * Ctxfn_i[1,ip] 
#                    yCt_i[-1,jp] -= s['dsz'][-1,jp] * ufn[-1,jp] * s['Cu0'][-1,jp,i] * p['sedimentinput'] #upper y-face
#                    yCt_i[-1,jm] -= s['dsz'][-2,jm] * ufn[-2,jm] * Ctxfn_i[-2,jm] 
                    ...
                    #remove the flux at the inner face of the cell
                    yCt_i[-1,:] -= s['dsz'][-2,:] * ufn[-2,:] * Ctxfn_i[-2,:] #lower y-face
                    if qflag :
                ...
                if qflag :
                    qs_i = qs[:,:,i].flatten()
                    qs_i += scipy.sparse.linalg.spsolve(A, yqs_i.flatten())
                    qn_i = qn[:,:,i].flatten()
                    qn_i += scipy.sparse.linalg.spsolve(A, yqn_i.flatten())
...
#                                           **logprops))
                ...
                if qflag :
                    qs[:,:,i] = qs_i.reshape(yqs_i.shape)###
                    qn[:,:,i] = qn_i.reshape(yqn_i.shape)###
